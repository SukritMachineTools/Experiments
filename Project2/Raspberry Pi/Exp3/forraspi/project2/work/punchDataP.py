...
class punchData():
    ...
    def punchDataFrame(self):
        ...
        homeimg = ImageTk.PhotoImage(
            Image.open('home.png').resize((150, 80), Image.LANCZOS))
        # settinglabel=Label(frame6,image=settingimg).grid(row=0,column=0,padx=50)
        Button(frame6, image=homeimg, command=pBrake, highlightthickness=0,borderwidth=0,bg="#b3ffe0").grid(row=0, column=0, padx=50)
        pulimg = ImageTk.PhotoImage(
            Image.open('puchlist.png').resize((150, 80), Image.LANCZOS))
        # pdlabel=Label(frame6,image=pdimg).grid(row=0,column=1,padx=50,columnspan=2)
        Button(frame6, image=pulimg, command=pnList, highlightthickness=0,borderwidth=0,bg="#b3ffe0").grid(row=0, column=1, padx=50, columnspan=2)
        opimg = ImageTk.PhotoImage(
            Image.open('operation.png').resize((150, 80), Image.LANCZOS))
        # pllabel=Label(frame6,image=plimg).grid(row=0,column=4,padx=50,columnspan=2)
        Button(frame6, image=opimg, command=op,highlightthickness=0,borderwidth=0,bg="#b3ffe0").grid(row=0, column=4, padx=50, columnspan=2)
